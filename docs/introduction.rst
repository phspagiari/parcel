...
application deployment using native linux packages. This project is an attempt to formalise some
of those ideas into a more generic installation framework that follows those guidelines.
...
When the build host needs a new version of code, the code is copied across to the build machine. It is `not` checked out on
the build machine. When completed packages are pushed into repositories, it is done from the local machine, not from the build host.
...
to be moved onto a build host to enable it to connect further. Once you have an ssh agent with your keys installed everything
runs seamlessly.

It also helps keep the software that needs to be installed on the build host to a minimum. For example, mercurial or git does
not need to be installed on the build host.
